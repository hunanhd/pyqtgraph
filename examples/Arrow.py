...
a2 = pg.ArrowItem(angle=180, tipAngle=30, baseAngle=20, headLen=40, tailLen=40, tailWidth=8, pen=None, brush='y',pixMode = False)
...
c = p2.plot(x=np.sin(np.linspace(0, 2*np.pi, 1000)), y=np.cos(np.linspace(0, 6*np.pi, 1000)),pixMode = False)
p2.hideAxis('left')
p2.hideAxis('bottom')
a = pg.CurveArrow(c,pixMode = False)
